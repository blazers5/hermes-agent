...
| `/bridge bind telegram [--chat CHAT_ID] [--user USER_ID] [--ttl SECONDS]` | Safe CLI→Telegram bridge opt-in. The local CLI mints a short-lived single-use token for the current session; send `/bridge_bind <token>` in a Telegram DM to complete the binding. After binding, plain Telegram DM text continues the linked CLI-originated Hermes session through the normal gateway path. Slash commands keep normal gateway semantics. `/bridge off` creates a filesystem kill switch, and `/bridge on` removes it. |
